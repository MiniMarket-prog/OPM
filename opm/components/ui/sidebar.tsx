...
import { useIsMobile as useActualIsMobile } from "@/hooks/use-mobile"
...
>(({ defaultOpen = true, open: openProp, onOpenChange: setOpenProp, className, style, children, ...props }, ref) => {
  ...
  const actualIsMobile = useActualIsMobile()

  React.useEffect(() => {
    setMounted(true)
  }, [])

  const isMobile = mounted ? actualIsMobile : false
...
const SidebarHeader = React.forwardRef<HTMLDivElement, React.ComponentProps<"div">>(({ className, ...props }, ref) => {
  ...
  return (
    <div
      ...
      data-sidebar="header"
      className={cn(
        "flex items-center gap-2 p-2 border-b border-sidebar-border h-[var(--header-height)]",
  ...
  return (
    <div
      ...
      className={cn(
        ...
        state === "collapsed" ? "px-0 items-center" : "px-2",
        className,
      )}
      {...props}
    />
...
const sidebarMenuButtonVariants = cva(
  // Combined Base classes + Icon mode specific overrides:
  "peer/menu-button flex w-full items-center overflow-hidden rounded-md p-2 text-sm outline-none ring-sidebar-ring transition-[width,height,padding] hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-[[data-sidebar=menu-action]]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-sidebar-accent data-[active=true]:font-medium data-[active=true]:text-sidebar-accent-foreground data-[state=open]:hover:bg-sidebar-accent data-[state=open]:hover:text-sidebar-accent-foreground" +
    " group-data-[collapsible=icon]:!size-10 group-data-[collapsible=icon]:!p-2 group-data-[collapsible=icon]:gap-0 group-data-[collapsible=icon]:text-center group-data-[collapsible=icon]:!justify-center group-data-[collapsible=icon]:[&>span]:hidden",
  {
    // Config object
    variants: {
      variant: {
        default: "text-left gap-2",
        outline:
          "text-left gap-2 bg-background shadow-[0_0_0_1px_hsl(var(--sidebar-border))] hover:bg-sidebar-accent hover:text-sidebar-accent-foreground hover:shadow-[0_0_0_1px_hsl(var(--sidebar-accent))]",
      },
      size: {
...
>(({ className, showIcon = false, ...props }, ref) => {
  ...
  return (
    <div
      ...
      className={cn(
        "rounded-md h-10 flex gap-2 px-2 items-center",
        state === "collapsed" && "group-data-[collapsible=icon]:justify-center group-data-[collapsible=icon]:px-0",
        className,
      )}
      {...props}
    >
